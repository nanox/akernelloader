...
int fatfs_split_path(char *full_path, char *path, int max_path, char *filename, int max_filename)
{
    ...
    {
        strindex = (int)strlen_akel(full_path) - (int)strlen_akel(filename);
        if (strindex > max_path)
            strindex = max_path;

        memmove_akel(path, full_path, strindex);
        path[strindex-1] = '\0';
    }
...
int fatfs_compare_names(char* strA, char* strB)
{
    ...
    if (ext1Pos!=-1)
    {
        ...
        if (strlen_akel(ext1) != strlen_akel(ext2))
            return 0;
        
        // If they dont match
        if (FileString_StrCmpNoCase(ext1, ext2, (int)strlen_akel(ext1))!=0) 
            return 0;
    ...
    else
    {
        ...
        file1Len = (int)strlen_akel(strA);
        file2Len = (int)strlen_akel(strB);
    }
